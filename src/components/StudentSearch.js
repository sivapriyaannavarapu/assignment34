...
//     setTimeout(() => {
//       setLoading(false);
//       navigate('/student', { state: { studentId: searchTerm.trim() } }); // Navigate to /student with state
//     }, 800);
//   }
// };
//   return (
...
//                 </div>
//                 <div>
//                   <p className="mb-0 text-muted alert-text" style={{ width: '350px' }}>{alertText}</p>
//                 </div>
//               </div>
//             </Col>
//           ))}
...
const StudentProfile = () => {
  ...
  useEffect(() => {
    if (!isStudentLoading && studentData && !studentError) {
      const newProfile = {
        id: String(studentData.studentId),
      ...
      setRecentSearches((prev) => {
        const updated = [newProfile, ...prev.filter((p) => p.id !== newProfile.id)].slice(0, 8);
  ...
  }, [studentData, isStudentLoading, studentError]);

  const handleIdSearch = () => {
    setShowRecentSearches(true); // Show recent searches
    setShowFilter(false); // Close filter toggle after clicking search
    setLoading(true); // Start loading to show skeleton cards

    if (!searchTerm.trim()) {
      // For empty search, show skeleton briefly before recent searches
      setTimeout(() => {
        setLoading(false);
        setTriggerSearch(false);
        setSearchResults([]);
    ...
    // For non-empty search, perform the search
    setSearchResults([]);
    setTriggerSearch(true);
    setTimeout(() => {
      setLoading(false);
      navigate(`/student/${searchTerm}`);
    }, 1000); // Delay for non-empty search
  };
  ...
  return (
    <main >
      <div className="me-3">
            ...
            <Row className="w-100">
              <Col md={12}>
                <div className="text-start p-2 w-100 me-4 pb-4 pt-3">
                  <h2 className="mb-2" style={{ fontSize: '36px', fontWeight: '700', lineHeight: '40px', letterSpacing: '-0.792px', color: 'rgba(37, 44, 50, 1)' }}>
                    Student Profile
                  </h2>
                  ...
                  <div className="position-relative search-filter-wrapper" style={{ width: '36%' }}>
                    <div className="input-group search-input-group w-100">
                      <span className="input-group-text bg-white border-end-0" style={{ cursor: 'pointer' }} onClick={() => setShowFilter(!showFilter)}>
                    ...
                    {showFilter && (
                      <div className="filter-toggle-box shadow" style={{ width: '100%', padding: '1rem' }}>
                        ...
                        <div className="mt-4 d-flex justify-content-center">
                          <Button className="w-25 search-btn" onClick={handleIdSearch}>
                            Search
                          </Button>
                        </div>
                      </div>
          ...
          <section className="py-2 mt-2 me-3">
            ...
            <Row className="g-4">
              {loading ? (
                ...
                (searchResults.length > 0 ? searchResults : recentSearches).slice(0, 8).map((profile) => (
        ...
        {!showRecentSearches && (
          <section
            className={`alerts-container border rounded mt-2 p-4 bg-white shadow-sm ${showFilter || loading ? 'disabled' : ''}`}
          >
            ...
            <Row>
              {[1, 2, 3].map((item, index) => (
                <Col key={index} md={4} className={`px-4 ${index !== 2 ? 'border-end' : ''}`}>
